...
from magpylib_material_response.customMesh import (
    import_mesh
)
...
def test_mesh_Cuboid():
    ...
    cm = import_mesh("tests/testdata/cuboid1.inp",scaling=1e-3, polarization=(0,0,1), succeptibility=3999)
    cm.move((0, 0,  -0.0005))

    # test if custom meshed cuboid collection provides the same field as original cuboid
    np.testing.assert_allclose(c.getB([0, 0, 0]), cm.getB([0, 0, 0]),rtol=7e-4,atol=2e-5)
...
def test_mesh_Cuboid2():
    ...
    c = import_mesh("tests/testdata/cuboid1.inp",scaling=1e-3, polarization=(0,0,1), succeptibility=3999)
    num_elemets = 80
